...
| `maxContentChars` | `number`                                   | `6000`                       | Truncate any single event's text, structured input, or output longer than this with a head + marker + tail replacement, so the validator still sees both edges.                                                 |
